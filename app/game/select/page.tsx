...
export default function CardManagementScreen({
  ...
  useEffect(() => {
    const fetchShopData = async () => {
      try {
        ...
        const res = await fetch(`https://gojoapi.onrender.com/shop/${shop_id}`);
  ...
  const calculatePrize = () => {
    ...
    const rate = commission_rate != null ? commission_rate : 0.2; // default 10% if undefined
  ...
  const startGame = async () => {
    ...
    try {
      ...
      const res = await fetch("https://gojoapi.onrender.com/savegame", {
        method: "POST",
